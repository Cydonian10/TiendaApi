...
- Endpoints de `BaseProductUnit` (tabla `base-product-unit`) anidados bajo `/base-products/:baseProductId/units`:
...
- `:unitId` = id de `measurement-unit` (no el id de la fila `base-product-unit`).
...
- Migración nueva agregando a `measurement-unit` los unique de `name` y `value`, y a `base-product-unit` el unique parcial `(baseProductId) WHERE isMain` (las tablas ya existen por la migración `Unidades` aplicada).
...
export class MeasurementUnitFilterDto extends PaginationDto {
  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ example: 'kilo' })
  search?: string; // unaccent sobre name
}
```
...
- El módulo hace `TypeOrmModule.forFeature([MeasurementUnit, BaseProductUnit, BaseProduct])` — valida existencia de base-product y unidad con sus repos (patrón de `ImagesModule` con `Product`).
...
- Unique parcial `WHERE "isMain"` → el service degrada la principal anterior (`UPDATE ... SET "isMain"=false WHERE "baseProductId"=... AND "isMain"`) **antes** del insert/update con `isMain:true`. Si igual viola → **409**.
- `:unitId` en rutas = `measurement-unit.id`; la fila `base-product-unit` se localiza por `(baseProductId, unitId)`.

## Implementation plan

1. Modificar entidades `MeasurementUnit` (índices únicos `name`, `value`) y `BaseProductUnit` (índice único parcial `baseProduct WHERE "isMain"`).
2. Migración nueva (p. ej. `MeasurementUnitIndexes`): agrega unique `name` y `value` en `measurement-unit`, y unique parcial `(baseProductId) WHERE "isMain"` en `base-product-unit` (las tablas ya existen por `Unidades`); aplicar con `npm run migration:run`.
...
- [x] `POST /measurement-units { name, value }` responde **201** con `MeasurementUnitDto`.
- [x] `name` repetido → **409**; `value` repetido → **409**.
- [x] `GET /measurement-units?search=...&page&limit` devuelve `PaginatedResult` con `search` unaccent sobre `name`.
- [x] `GET /measurement-units/:id` inexistente → **404**.
- [x] `PATCH /measurement-units/:id` actualiza `name`/`value`; unique → **409**; inexistente → **404**.
- [x] `DELETE /measurement-units/:id` borra la unidad; si está referenciada por `base-product-unit` → **409**; inexistente → **404**.
- [x] `GET /base-products/:baseProductId/units` lista las unidades del base-product; baseProduct inexistente → **404**.
- [x] `POST /base-products/:baseProductId/units { unitId, factor?, isMain? }` asocia la unidad → **201**; baseProduct inexistente → **404**; `unitId` inexistente → **404**; unidad ya asociada → **409**.
- [x] `factor` omitido → queda `1`; `factor <= 0` → **400**.
- [x] `isMain:true` en add queda como principal y la principal anterior (si existía) pasa a `isMain:false`.
- [x] `PATCH /base-products/:baseProductId/units/:unitId { factor?, isMain? }` edita los campos; `isMain:true` degrada la principal anterior; unidad no perteneciente al base-product → **404**.
- [x] `DELETE /base-products/:baseProductId/units/:unitId` desasocia la unidad; no perteneciente → **404**; si era `isMain` → **409**.
- [x] En BD existe `measurement-unit` (unique `name`, unique `value`) y `base-product-unit` (unique `(baseProductId, unitId)`, unique parcial `(baseProductId) WHERE "isMain"`).
...
- **Sí:** se mantiene la entidad/tabla `BaseProductUnit`/`base-product-unit` ya existente y aplicada en BD (renombrada en el commit del spec); se ajusta el spec a esa realidad en lugar de renombrar de nuevo. El API no cambia: `/base-products/:baseProductId/units`.
- **No:** no controlar `isMain` solo desde el service (se descarta por la race condition; gana el unique parcial).
- **No:** `:unitId` como id de `base-product-unit` (se descarta por la semántica de "validar que la unidad sea del base-product").
...
| Borrar `measurement-unit` en uso por `base-product-unit` deja FK colgada. | Capturar `isForeignKeyViolation` → 409 en `DELETE /measurement-units/:id`.                      |
...
| `base-product-unit` sin `isMain` queda sin principal tras operaciones.   | Aceptado: el cliente marca una con PATCH; no auto-promueve.                                      |
